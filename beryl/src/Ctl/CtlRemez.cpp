...
// $Id: CtlRemez.cpp 713 2023-02-13 01:30:20Z duncang $
...
void        CtlRemez::SetPbEdge      ( double  i_f ) {
  double xbw;
...
void        CtlRemez::Doit           ( void         ) {
  ...
  for(llong c=0; c<numCh; c++) {
    s = ary[c];
    mdRemez->SetAry((ldouble *)s); // FIXME This is a total hack since I was experimenting with long double arrays to get Remez to converge better.
    mdRemez->Calculate();
    mdRemez->NormPeak();
  }
  return;
