...
// $Id: CtlSine.cpp 572 2021-06-21 00:42:24Z duncang $
...
void         CtlSine::SetFreqSine        ( ullong i_c, double i_f    ) {
  if(i_f < 0.0)                  i_f = 0.0;
  if(i_f > sig->GetFS() * 0.5) i_f = sig->GetFS() * 0.5;
  if(i_c < 0) i_c = 0;
...
void         CtlSine::DoIt               ( void          ) {
  ...
  ch   = sig->GetCh();
  N    = sig->GetN();
  dt   = PIx2;             // One tick is 1 cycle in radians
  dt  /= sig->GetFS();   // but each tick is actually pretty fast
  dt  *= fSine[1];            // and finally, we are typically far from 1 Hz.
