// $Id: CfgStrings.cpp 763 2023-03-25 01:24:59Z duncang $
...
CfgStrings::CfgStrings(const char *theLang, int buildNo) {
  ...
  char tStr[32768];
  // FIXME This hack needs to be internationalized.  Either that or strip out internationalization.
  strcpy(tStr, "Exit the agate application."       );   strsAppExit        = new char[strlen(tStr) + 1]; strcpy(strsAppExit,        tStr);
  strcpy(tStr, "Choose a file for analysis."          );   strsFileInSel      = new char[strlen(tStr) + 1]; strcpy(strsFileInSel,      tStr);
  strcpy(tStr, "Parse the file's RIFF Structure"      );   strsFileInParse    = new char[strlen(tStr) + 1]; strcpy(strsFileInParse,    tStr);
  strcpy(tStr, "Choose a file for Processed Wave."    );   strsFileOutSel     = new char[strlen(tStr) + 1]; strcpy(strsFileOutSel,     tStr);
  strcpy(tStr, "There is nothing to save right now."  );   strsFileSaveNone   = new char[strlen(tStr) + 1]; strcpy(strsFileSaveNone,   tStr);
	...
	strcpy(en_PROG_NAME                , "agate");
	strcpy(en_MAIN_TITLE               , "agate Audio File Analyzer");
	strcpy(en_CARD_CONFIG_TITLE        , "agate - Configure Sound Card");
	strcpy(en_CARD_CONFIG_ST_CHG_CARD  , "Card changed:  ");
	strcpy(en_ABOUT_TITLE              , "About agate");
	strcpy(en_ABOUT_PROG               , "agate Audio File Analyzer");
	...
	strcpy(fr_PROG_NAME                , "agate");
	strcpy(fr_MAIN_TITLE               , "agate analyzeur de dosier d'audio");
	strcpy(fr_CARD_CONFIG_TITLE        , "agate - Configurer carte son");
	strcpy(fr_CARD_CONFIG_ST_CHG_CARD  , "Carte changé:  ");
	strcpy(fr_ABOUT_TITLE              , "À agate");
	strcpy(fr_ABOUT_PROG               , "agate analyzeur de dosier d'audio");
	...
	strcpy(zz_PROG_NAME                , "agate");
	strcpy(zz_MAIN_TITLE               , "agate Audio File Analyzer");
	strcpy(zz_CARD_CONFIG_TITLE        , "agate - Configure Sound Card");
	strcpy(zz_CARD_CONFIG_ST_CHG_CARD  , "Card changed:  ");
	strcpy(zz_ABOUT_TITLE              , "About agate");
	strcpy(zz_ABOUT_PROG               , "agate Audio File Analyzer");
